...
func init() {
	...
	if err != nil {
		panic(fmt.Sprintf("parsing public suffix list", err))
	}
	publicsuffixList = l
}
...
func Lookup(elog *slog.Logger, domain dns.Domain) (orgDomain dns.Domain) {
	return publicsuffixList.Lookup(elog, domain)
}
...
func (l List) Lookup(elog *slog.Logger, domain dns.Domain) (orgDomain dns.Domain) {
	defer func() {
		elog.Debug("publicsuffix lookup result", slog.Any("reqdom", domain), slog.Any("orgdom", orgDomain))
	}()
