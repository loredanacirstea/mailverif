...
func TestSignARC(t *testing.T) {
	...
	header, err := Sign(logger, identif, domain, selectors, false, r, timeNow)
	require.NoError(t, err)

	slices.Reverse(header)
	newemail := strings.Join(header, "") + signedMailString
	...
	results, err := Verify(logger, &DNSResolverTest{}, false, msgr, false, true, timeNow, publicKey)
	require.NoError(t, err)
	require.NoError(t, results.Result.Err)
	require.Equal(t, dkim.StatusPass, results.Result.Status)
	require.Equal(t, 1, len(results.Chain))
